  ...
  return (
    ...
    >
          ...
          if (item.to === '/mision-inventario') {
            return userCanSeeInventoryMissions(user) || userCanSeeMissionProgressOnly(user) ? (
              <NavItem
                ...
                to={item.to}
                icon={item.icon}
                label={userCanSeeMissionProgressOnly(user) && !userCanSeeInventoryMissions(user) ? 'Progreso inventario' : item.label}
              />
            ) : null;
          }
      ...
      <div className="shrink-0 border-t border-slate-200/80 p-2 dark:border-slate-800/50 xl:p-4">
        ...
        {user ? (
          <div className="mt-2 border-t border-slate-200/80 pt-2 xl:hidden dark:border-slate-800/50">
