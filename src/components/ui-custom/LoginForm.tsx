...
function PinKeypadGrid({
  ...
  const keys: { label: string; onClick: () => void; className?: string }[] = [
    ...
    { label: '9', onClick: () => append('9') },
    {
      label: 'C',
      onClick: clear,
      className: 'text-sm font-bold text-brand-to dark:text-brand',
    },
...
export function LoginForm() {
  ...
  return (
    ...
    >
      <div className="fixed right-[max(0.75rem,env(safe-area-inset-right,0px))] top-[max(0.75rem,env(safe-area-inset-top,0px))] z-20 flex items-center gap-1.5">
        <Button
          type="button"
          variant="ghost"
          size="icon"
          className="h-10 w-10 rounded-xl bg-white/80 text-slate-700 shadow-sm backdrop-blur-md hover:bg-white dark:bg-slate-800/80 dark:text-slate-200 dark:hover:bg-slate-800"
          onClick={() => toggleTheme()}
        ...
        <AccentColorPicker className="bg-white/80 shadow-sm backdrop-blur-md hover:bg-white dark:bg-slate-800/80 dark:hover:bg-slate-800" />
      </div>
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
        <div className="absolute -left-1/4 top-0 h-[min(100dvh,56rem)] w-[min(140vw,56rem)] rounded-full bg-brand/20 blur-3xl dark:bg-brand-from/10" />
        <div className="absolute -right-1/4 bottom-0 h-[min(100dvh,52rem)] w-[min(130vw,52rem)] rounded-full bg-brand-to/15 blur-3xl dark:bg-brand-to/10" />
        <div className="absolute left-1/2 top-1/2 h-[min(90dvh,40rem)] w-[min(90vw,40rem)] -translate-x-1/2 -translate-y-1/2 rounded-full bg-brand/10 blur-3xl dark:bg-brand/15" />
      </div>

      <div className="relative z-10 w-full min-w-0 max-w-md">
        <div className="absolute -inset-1 rounded-2xl bg-brand-gradient opacity-25 blur" />
        ...
        >
          <div className="mb-5 flex flex-col items-center sm:mb-6">
            <div className="mb-3 flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl shadow-lg shadow-brand/15 ring-1 ring-slate-300/80 dark:shadow-brand/20 dark:ring-slate-700/50 sm:mb-4 sm:h-24 sm:w-24">
              <img
                src={BRAND_LOGO_URL}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex min-w-0 rounded-md border border-slate-300 bg-slate-50/80 focus-within:border-brand/50 focus-within:ring-1 focus-within:ring-brand/20 dark:border-slate-700 dark:bg-slate-900/50">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  disabled={formBusy}
                  className={cn(
                    'h-10 border-slate-300 bg-slate-50/80 pl-10 font-mono tracking-widest text-slate-900 placeholder:text-slate-500 focus:border-brand/50 focus-visible:ring-brand/20 dark:border-slate-700 dark:bg-slate-900/50 dark:text-slate-100 dark:placeholder:text-slate-600',
            ...
            <Button
              ...
              className={cn(
                ...
                'bg-brand-gradient hover:from-brand-from hover:to-brand-to',
                'text-white shadow-lg shadow-brand/25 hover:shadow-brand/40',
